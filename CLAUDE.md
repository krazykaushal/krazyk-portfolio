...
- [x] **Phase 6 — Enhancements** (independent of each other; tackle in any order, one at a time). See notes below.
  - [x] **Social links** — GitHub, LinkedIn, Resume in `components/ui/SocialLinks.tsx` (server component). Mounted in the sticky avatar lane (`app/page.tsx`): a horizontal icon row under the avatar on mobile, a vertical rail on the avatar's right from `md` up. Links carry `data-avatar-react="rail"`, which the avatar's hover hook branches on for a distinct "look over" reaction (head turns right + eyebrow waggle via GSAP keyframes). Resume URL comes from `NEXT_PUBLIC_RESUME_URL` (omitted if unset).
  - [x] **Hero background animation** — `components/ui/HeroBackground.tsx`, a canvas adaptation of reactbits "Squares" (no dependency): a faint grid drifting diagonally. Rendered outside `<main>` so it's full-bleed; `absolute inset-x-0 top-0 -z-30 h-svh`, masked to fade out before About. Line color reads `--foreground` and re-reads on theme toggle (MutationObserver). Ambient/non-interactive; reduced-motion draws one static grid (canvas rAF gated in JS).
  - [x] **Hero text intro** (bonus) — on-load sequence in `components/sections/Hero.tsx` (`'use client'`): eyebrow fades in, name rises char-by-char (GSAP SplitText stagger), role line decrypts via ScrambleText, CTAs fade in. SplitText/ScrambleText (free in GSAP 3.13+) registered in `lib/gsap.ts` — no new dependency. `aria:"auto"` + an `sr-only` role copy keep it accessible; reduced-motion shows static text; `gsap.from` immediate-render avoids FOUC.
  - [x] **About as a terminal UI** — `components/sections/About.tsx` reskinned as a terminal window (traffic-light chrome, `kaushal@portfolio: ~/about` title, monospace) and now a `'use client'` island. Types out commands (`whoami`, `cat highlights.txt`) char-by-char on scroll-in (IntersectionObserver), revealing each real output after. Reduced-motion shows full output at once; outputs stay in the DOM (opacity-only animation) for SR/crawlers + no layout shift. Blinking cursor via `animate-pulse`.
  - [x] **Light-mode toggle** — code-flag switch (`darkMode ●━○ true`) in `components/ui/ThemeToggle.tsx`, fixed top-right. Holds no React state: knob/label are pure CSS via `dark:` variants, so no hydration mismatch. Click toggles `.dark` on `<html>` + writes `localStorage.theme`. A pre-paint inline script in `layout.tsx` reapplies the saved choice (falling back to OS `prefers-color-scheme`) before paint to avoid FOUC; `<html>` has `suppressHydrationWarning`. Avatar's light backdrop disc gated to `dark:block` (`app/page.tsx`).
